...
EVENT_RESOURCE(resource_alarm, "title=\"alarm\";rt=\"Text\"", alarm_get_handler, NULL, alarm_put_handler, NULL, alarm_event_handler);
...
static void alarm_get_handler(void* request, void* response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset) {
	...
	sprintf(message, "{'alarm':'%d'}", node_alarm);
...
static void alarm_event_handler() {
	REST.notify_subscribers(&resource_alarm);
}